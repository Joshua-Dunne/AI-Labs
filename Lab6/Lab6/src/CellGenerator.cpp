...
void CellGenerator::generateNodes()
{
	...
	for (int yPos = 0; yPos < c_MAX_Y; yPos++)
	{
		for (int xPos = 0; xPos < c_MAX_X; xPos++)
		{
			m_data[yPos][xPos]->m_name = currentNode;
			m_graph.addNode(*m_data[yPos][xPos], currentNode);
			currentNode++;
		}
	}
}
		...
		for (int col = 0; col < c_MAX_X; col++)
		{
			for (int direction = 0; direction < 9; direction++) {
				...
				if (n_row >= 0 && n_row < c_MAX_Y && n_col >= 0 && n_col < c_MAX_X) {
					...
					The cost will be the square root of the combined sizes, which is sqrt(64).
					...
					So we will have the nodes next to have 32, and diagonal be sqrt(32 + 32).
					*/
