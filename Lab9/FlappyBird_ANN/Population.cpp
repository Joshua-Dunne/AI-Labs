...
void Population::evolve()
{
	...
	{
		startingCount++;
		newBirdSet[0] = birdSet[fittest];
	}

	for (; startingCount < Game::birdCount; startingCount++)
	{
...
std::vector<std::pair<int, float>> Population::getTopFittest(int t_num)
{
	int index = t_num;
	int fittest = birdSet[0].fitness;
	...
	{
		if (birdSet[index].fitness > birdSet[top[t_num - 1].first].fitness)
		{ // if the fitness of this bird is greater than the smallest fitness of our top birds
			// we will check to see if it is greater than the other birds
			for (int i = t_num - 1; i >= 0; i--)
			{ // start at the back
				if (birdSet[index].fitness < birdSet[top[i].first].fitness)
				{ // if the bird's fitness is smaller than the current bird's fitness
				  // that means we have reached where the new bird should be inserted
					
					
					// re-sort bird fitness
