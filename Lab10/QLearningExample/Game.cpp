...
void Game::placeObstacle(sf::Vector2i t_mouseCoords)
{
	int x = gridWorld.getGridCellX(t_mouseCoords);
	int y = gridWorld.getGridCellY(t_mouseCoords);

	x--; y--; // x and y are always 1 greater than the mouse click, so go -1 on both here

	if (gridWorld.nodes && gridWorld.nodes[x]) // if both exist
	{
		bool obstacle = gridWorld.nodes[x][y].containsObstacle;

		if (obstacle)
		{
			gridWorld.nodes[x][y].containsObstacle = false;
			gridWorld.nodes[x][y].setColor(sf::Color::Green);
		}
		else if (!obstacle)
		{
			gridWorld.nodes[x][y].containsObstacle = true;
			gridWorld.nodes[x][y].setColor(sf::Color::Black);
		}
	}
	else
		std::cout << "Failed to place obstacle @ " << x << ", " << y << std::endl;
}
...
void Game::setupDroids()
{
	...
	Routine* moveTo2 = new MoveTo(1, 1, gridWorld);		// Just set any initial behaviour
														// In this version the real MoveTo behaviour will be determined by the position of the goalNo (Healthpill #)
